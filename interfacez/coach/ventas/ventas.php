...
$cantidad_registros = $conexion->query("SELECT COUNT(*) as total FROM ventas WHERE vendedor = '$documento'")->fetch()['total'];
...
$sentencia = $conexion->query("SELECT (SELECT nombre FROM usuarios WHERE documento = ventas.vendedor) AS vendedor,ventas.total, ventas.fecha, ventas.id, GROUP_CONCAT(productos.codigo, '..', productos.descripcion, '..', productos_vendidos.cantidad SEPARATOR '__') AS productos, nombre, ventas.doc_cliente FROM ventas INNER JOIN productos_vendidos ON productos_vendidos.id_venta = ventas.id INNER JOIN productos ON productos.id = productos_vendidos.id_producto INNER JOIN usuarios ON usuarios.documento = ventas.doc_cliente AND DATE(fecha) = CURDATE() AND vendedor = '$documento' GROUP BY ventas.id ORDER BY ventas.id DESC;");
        ...
        <ul class="nav nav-tabs justify-content-center">
            <li class="nav-item">
                <a class="nav-link text-light p-3 btn" aria-current="page"
                    href="../../coach/index-coach.php"><b>MENU</b></a>
            </li>
            <li class="nav-item">
            ...
            <nav aria-label="Page navigation">
                <ul class="pagination justify-content-center">
                    <?php if ($pagina_actual > 1): ?>
                        <li class="page-item"><a class="page-link" href="?pagina=<?= $pagina_actual - 1 ?>">Anterior</a>
                        </li>
                    <?php endif; ?>
                    ...
                    <?php if ($pagina_actual < $paginas_totales): ?>
                        <li class="page-item"><a class="page-link" href="?pagina=<?= $pagina_actual + 1 ?>">Siguiente</a>
                        </li>
                    <?php endif; ?>
                </ul>
            </nav>
