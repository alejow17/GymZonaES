...
$buscador = $conexion->prepare("SELECT (SELECT nombre FROM usuarios WHERE documento = ventas.vendedor) AS vendedor, nombre, doc_cliente, ventas.total, ventas.fecha, ventas.id, GROUP_CONCAT(productos.codigo, '..', productos.descripcion, '..', productos_vendidos.cantidad SEPARATOR '__') AS productos FROM ventas INNER JOIN productos_vendidos ON productos_vendidos.id_venta = ventas.id INNER JOIN productos ON productos.id = productos_vendidos.id_producto INNER JOIN usuarios ON usuarios.documento = ventas.doc_cliente AND vendedor = '$documento' AND nombre LIKE LOWER('%".$_POST["buscar"]."%') GROUP BY ventas.id ORDER BY ventas.id");
$buscador->execute();
